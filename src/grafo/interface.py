...
from .visualizacao import cores_relacionamentos, preparar_desenho
...
def _carregar_template() -> str:
    return "\n".join(
        [
            "# Relacione cada pessoa ao filme que assistiu usando 'x'",
            "# Os nomes podem conter espaços e acentos",
            "Gabriel x Moana",
            "Gabriel x Frozen",
            "Larissa x Up altas aventuras",
            "Marcelo x Moana",
        ]
    )
...
class VisualizadorBipartido(QMainWindow):
    ...
    def _carregar_exemplo_padrao(self) -> None:
        ...
        preferidos = [
            "usuarios_filmes_recomendacao.txt",
            "usuarios_filmes_conflito.txt",
    ...
    def _atualizar_relacionamentos(self) -> None:
        ...
        paleta = cores_relacionamentos(self._resultado)

        linhas = []
        for vertice in sorted(adjacencias.keys()):
            vizinhos = ", ".join(sorted(adjacencias[vertice])) or "nenhuma conexão"
            cor = paleta.get(vertice, "#374151")
            marcador = f'<span style="color:{cor}">●</span>'
    ...
    def _desenhar_grafo(self) -> None:
        ...
        if self._layout_atual == "bipartido" and resultado.eh_bipartido:
            ...
            if xs_a and xs_b:
                margem = 0.12
                ax.axvspan(min(xs_a) - margem, max(xs_a) + margem, color="#e3f2ff", alpha=0.7, zorder=0)
                ax.axvspan(min(xs_b) - margem, max(xs_b) + margem, color="#fff0e1", alpha=0.7, zorder=0)

        nx.draw_networkx_edges(grafo_nx, posicoes, ax=ax, edge_color=cores_arestas, width=2.6)
        ...
        legenda_itens = [
            ("🟦", "Partição 0"),
            ("🟧", "Partição 1"),
            ("🔴", "Aresta em conflito" if resultado.conflitos else "Relacionamentos coloridos"),
        ]
...
Gabriel x Moana
Gabriel x Frozen
Larissa x Up altas aventuras</pre>
        """
